...
class DiTBlock(nn.Module):
    def __init__(
        ...
        ffn_dim: int,
        eps: float = 1e-6,
        # OmniShow-GLCA (default off)
        omnishow_enable_audio_inject: bool = False,
        omnishow_audio_inject_window_size: int = 5,
        omnishow_audio_gate_init: float = 1e-5,
    ):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.omnishow_enable_audio_inject = bool(omnishow_enable_audio_inject)
        if self.omnishow_enable_audio_inject:
            from diffsynth.modules.gated_local_context_attention import GatedLocalContextAttention

            self.glca = GatedLocalContextAttention(
                dim=dim,
                num_heads=num_heads,
                window_size=int(omnishow_audio_inject_window_size),
                gate_init=float(omnishow_audio_gate_init),
                eps=float(eps),
            )
    ...
    def forward(
        ...
        video_frame_lens: torch.Tensor | None = None,
        audio_frame_lens: torch.Tensor | None = None,
    ):
        ...
        # OmniShow-GLCA injection (after text cross-attn merge)
        if self.omnishow_enable_audio_inject and audio_tokens is not None:
            x = x + self.glca(x, audio_tokens, video_frame_lens, audio_frame_lens)
...
class WanModel(torch.nn.Module):
    ...
    def __init__(
        ...
        # OmniShow-GLCA (default off)
        omnishow_enable_audio_inject: bool = False,
        # For Wan, we align with Wan-S2V's default sparse audio injection layers.
        omnishow_audio_inject_layers = [0, 4, 8, 12, 16, 20, 24, 27, 30, 33, 36, 39],
        omnishow_audio_inject_window_size: int = 5,
        omnishow_audio_gate_init: float = 1e-5,
        omnishow_audio_packed_dim: int = 13 * 768,
    ):
        super().__init__()
        self.dim = dim
        self.in_dim = in_dim
        ...
        self.omnishow_enable_audio_inject = bool(omnishow_enable_audio_inject)
        self.omnishow_audio_inject_layers = list(omnishow_audio_inject_layers)
        self.omnishow_audio_inject_layers_set = set(int(i) for i in self.omnishow_audio_inject_layers)
        self.omnishow_audio_inject_window_size = int(omnishow_audio_inject_window_size)
        self.omnishow_audio_gate_init = float(omnishow_audio_gate_init)
        self.omnishow_audio_packed_dim = int(omnishow_audio_packed_dim)
        if self.omnishow_enable_audio_inject:
            self.omnishow_audio_projector = nn.Linear(self.omnishow_audio_packed_dim, dim)
        else:
            self.omnishow_audio_projector = None
        ...
        self.blocks = nn.ModuleList(
            [
                DiTBlock(
                    ...
                    ffn_dim,
                    eps,
                    omnishow_enable_audio_inject=(
                        self.omnishow_enable_audio_inject and (i in self.omnishow_audio_inject_layers_set)
                    ),
                    omnishow_audio_inject_window_size=self.omnishow_audio_inject_window_size,
                    omnishow_audio_gate_init=self.omnishow_audio_gate_init,
                )
    ...
    def forward(self,
        ...
        # OmniShow-GLCA kwargs (all optional)
        # - audio_tokens:          (B, T_latent, w, dim)  # already projected
        # - video_frame_lens/audio_frame_lens: (B, N_groups)
        audio_tokens = kwargs.get("audio_tokens", None)
        video_frame_lens = kwargs.get("video_frame_lens", None)
        audio_frame_lens = kwargs.get("audio_frame_lens", None)
