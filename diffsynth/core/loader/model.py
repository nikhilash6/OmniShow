...
def load_model(model_class, path, config=None, torch_dtype=torch.bfloat16, device="cpu", state_dict_converter=None, use_disk_map=False, module_map=None, vram_config=None, vram_limit=None, state_dict=None):
    ...
    # Control whether state-dict loading should allow architecture extensions.
    strict_env = os.environ.get("DIFFSYNTH_LOAD_STATE_DICT_STRICT", "true").strip().lower()
    strict = strict_env not in {"0", "false", "no", "n"}
    # What is `module_map`?
