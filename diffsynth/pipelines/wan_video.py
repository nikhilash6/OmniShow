...
# Fixed number of OmniShow reference pseudo-frames.
max_num_omnishow_ref_images = 2
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device=get_device_type(), torch_dtype=torch.bfloat16):
        ...
        self.units = [
            ...
            WanVideoUnit_OmniShow_Audio(),
            WanVideoUnit_S2V(),
            WanVideoUnit_InputVideoEmbedder(),
            WanVideoUnit_ImageEmbedderVAE(),
            WanVideoUnit_ImageEmbedderCLIP(),
            WanVideoUnit_ImageEmbedderFused(),
            WanVideoUnit_OmniShow_RefPose(),
    ...
    def from_pretrained(
        ...
        omnishow_audio_encoder: bool = False,
        omnishow_audio_out_dim: int = 1024,
        ...
        # Configure the audio encoder path for OmniShow.
        ...
        elif omnishow_audio_encoder:
            from diffsynth.modules.audio_encoder_omnishow import OmniShowAudioEncoder
            pipe.audio_encoder = OmniShowAudioEncoder(out_dim=int(omnishow_audio_out_dim))
    ...
    def __call__(
        ...
        is_omnishow_enable_ref: bool = False,
        # Speech-to-video
        input_audio: Optional[np.array] = None,
        audio_embeds: Optional[torch.Tensor] = None,
        audio_sample_rate: Optional[int] = 16000,
        is_omnishow_enable_audio: Optional[bool] = False,
        ...
        # Inputs
        # Build fixed-length OmniShow reference pseudo-frames when enabled.
        omnishow_reference_image = None
        if is_omnishow_enable_ref:
            omnishow_reference_image = ref_images
            if omnishow_reference_image is not None and not isinstance(omnishow_reference_image, list):
                omnishow_reference_image = [omnishow_reference_image]
            omnishow_reference_image = (omnishow_reference_image or [])[:max_num_omnishow_ref_images]

            ph, pw, _ = self.check_resize_height_width(height, width, num_frames, verbose=0)
            if len(omnishow_reference_image) < max_num_omnishow_ref_images:
                pad_num = max_num_omnishow_ref_images - len(omnishow_reference_image)
                pad_imgs = [Image.new("RGB", (pw, ph), (0, 0, 0)) for _ in range(pad_num)]
                omnishow_reference_image = omnishow_reference_image + pad_imgs
        inputs_posi = {
            "prompt": prompt,
        ...
        inputs_shared = {
            ...
            "omnishow_reference_image": omnishow_reference_image,
            "is_omnishow_enable_ref": is_omnishow_enable_ref,
            ...
            "input_audio": input_audio, "audio_sample_rate": audio_sample_rate, "is_omnishow_enable_audio": is_omnishow_enable_audio,
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:
                if cfg_merge:
                    ...
                    inputs_shared_nega = inputs_shared
                    if inputs_shared.get("omnishow_audio_embeds") is not None:
                        inputs_shared_nega = dict(inputs_shared)
                        inputs_shared_nega["omnishow_audio_embeds"] = None
        ...
        omnishow_ref = inputs_shared.get("omnishow_reference_image")
        if omnishow_ref is not None:
            f = len(omnishow_ref) if isinstance(omnishow_ref, list) else 1
            if f:
                inputs_shared["latents"] = inputs_shared["latents"][:, :, f:]
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(
        ...
        if vace_reference_image is not None:
            ...
            length += f
        if omnishow_reference_image is not None:
            # Add fixed-length OmniShow pseudo-frames to the latent timeline.
            length += int(max_num_omnishow_ref_images)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(
        ...
        if omnishow_reference_image is not None:
            # Encode OmniShow reference pseudo-frames before the input video latents.
            if not isinstance(omnishow_reference_image, list):
                omnishow_reference_image = [omnishow_reference_image]
            omnishow_reference_image = omnishow_reference_image[:max_num_omnishow_ref_images]

            if len(omnishow_reference_image) < max_num_omnishow_ref_images:
                pad_num = max_num_omnishow_ref_images - len(omnishow_reference_image)
                pad_w = pad_h = None
                if isinstance(input_video, list) and len(input_video) > 0 and isinstance(input_video[0], Image.Image):
                    pad_w, pad_h = input_video[0].size
                elif len(omnishow_reference_image) > 0 and isinstance(omnishow_reference_image[0], Image.Image):
                    pad_w, pad_h = omnishow_reference_image[0].size
                if pad_w is not None and pad_h is not None:
                    omnishow_reference_image = omnishow_reference_image + [
                        Image.new("RGB", (pad_w, pad_h), (0, 0, 0)) for _ in range(pad_num)
                    ]

            omnishow_reference_frames = pipe.preprocess_video(omnishow_reference_image)
            real_ref_count = 0
            if ref_images is not None:
                if isinstance(ref_images, list):
                    real_ref_count = len(ref_images)
                else:
                    real_ref_count = 1
            real_ref_count = max(0, min(int(real_ref_count), int(max_num_omnishow_ref_images)))
            if real_ref_count < int(max_num_omnishow_ref_images):
                omnishow_reference_frames[:, :, real_ref_count:] = 0
            ref_batch = omnishow_reference_frames.permute(2, 1, 0, 3, 4)
            ref_batch = ref_batch.to(dtype=pipe.torch_dtype, device=pipe.device)
            ref_latents = pipe.vae.encode(ref_batch, device=pipe.device)
            ref_latents = ref_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
            ref_latents = ref_latents.squeeze(2).permute(1, 0, 2, 3).unsqueeze(0)
            input_latents = torch.concat([ref_latents, input_latents], dim=2)
...
class WanVideoUnit_OmniShow_RefPose(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=(
                "ref_images",
                "pose_video",
                "is_omnishow_enable_ref",
                "num_frames",
                "height",
                "width",
                "tiled",
    ...
    def _normalize_ref_images(self, ref_images, width, height):
    ...
    def _pad_or_crop_latents(self, latents, target_length):
        if latents.shape[2] == target_length:
            return latents
        if latents.shape[2] > target_length:
            return latents[:, :, :target_length]
        pad_frames = target_length - latents.shape[2]
        pad = latents[:, :, -1:].repeat(1, 1, pad_frames, 1, 1)
        return torch.cat([latents, pad], dim=2)

    def _pad_or_crop_pixels(self, pixels: torch.Tensor, target_num_frames: int) -> torch.Tensor:
        """Pad/crop preprocessed pixel video to a fixed length.

        `pixels` is expected to be (1, 3, T, H, W) in the same preprocessed space as
        `pipe.preprocess_video`. Padding repeats the last frame (consistent with latent padding).
        """
        if pixels is None:
            return None
        if pixels.shape[2] == target_num_frames:
            return pixels
        if pixels.shape[2] > target_num_frames:
            return pixels[:, :, :target_num_frames]
        pad_frames = target_num_frames - pixels.shape[2]
        pad = pixels[:, :, -1:].repeat(1, 1, pad_frames, 1, 1)
        return torch.cat([pixels, pad], dim=2)

    def _build_i2v_like_mask_from_frame_mask(self, msk_1thw: torch.Tensor) -> torch.Tensor:
        """Match WanVideoUnit_ImageEmbedderVAE mask shaping.

        Input:  (1, T, h, w)
        Output: (4, (T-1)//4 + 1, h, w)
        """
        msk = torch.concat([torch.repeat_interleave(msk_1thw[:, 0:1], repeats=4, dim=1), msk_1thw[:, 1:]], dim=1)
        msk = msk.view(1, msk.shape[1] // 4, 4, msk.shape[2], msk.shape[3])
    ...
    def process(
        ...
        # Build the combined pose/reference conditioning tensor.
        if (not is_omnishow_enable_ref) and ref_images is None and pose_video is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)

        real_length = (num_frames - 1) // 4 + 1
        lat_h = height // pipe.vae.upsampling_factor
        lat_w = width // pipe.vae.upsampling_factor

        # Encode pose conditioning on the video-frame timeline.
        has_pose = pose_video is not None
        if has_pose:
            pose_pixels = pipe.preprocess_video(pose_video)
            pose_pixels = self._pad_or_crop_pixels(pose_pixels, num_frames)
        else:
            pose_pixels = torch.zeros((1, 3, num_frames, height, width), dtype=pipe.torch_dtype, device=pipe.device)

        pose_latents = pipe.vae.encode(
            [pose_pixels[0].to(dtype=pipe.torch_dtype, device=pipe.device)],
            device=pipe.device,
            tiled=tiled,
            tile_size=tile_size,
            tile_stride=tile_stride,
        )[0]
        pose_latents = pose_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
        if pose_latents.shape[1] != real_length:
            pose_latents = self._pad_or_crop_latents(pose_latents.unsqueeze(0), real_length)[0]

        pose_frame_mask = (
            torch.ones((1, num_frames, lat_h, lat_w), dtype=pipe.torch_dtype, device=pipe.device)
            if has_pose else
            torch.zeros((1, num_frames, lat_h, lat_w), dtype=pipe.torch_dtype, device=pipe.device)
        )
        pose_msk = self._build_i2v_like_mask_from_frame_mask(pose_frame_mask)

        if not is_omnishow_enable_ref:
            y = torch.cat([pose_msk, pose_latents], dim=0).unsqueeze(0)
            return {"y": y}

        # Encode fixed-length reference conditioning before real video frames.
        num_ref = int(max_num_omnishow_ref_images)
        normalized_ref_images = self._normalize_ref_images(ref_images, width, height) or []
        normalized_ref_images = normalized_ref_images[:num_ref]
        real_ref_count = len(normalized_ref_images)

        if real_ref_count > 0:
            real_ref_pixels = pipe.preprocess_video(normalized_ref_images)
            real_ref_pixels = real_ref_pixels.to(dtype=pipe.torch_dtype, device=pipe.device)
        else:
            real_ref_pixels = None

        ref_pixels = torch.zeros((1, 3, num_ref, height, width), dtype=pipe.torch_dtype, device=pipe.device)
        if real_ref_pixels is not None:
            ref_pixels[:, :, :real_ref_count] = real_ref_pixels

        ref_batch = ref_pixels.permute(2, 1, 0, 3, 4)
        ref_latents = pipe.vae.encode(ref_batch, device=pipe.device)
        ref_latents = ref_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
        ref_latents = ref_latents.squeeze(2).permute(1, 0, 2, 3)

        ref_msk = torch.zeros((4, num_ref, lat_h, lat_w), dtype=pipe.torch_dtype, device=pipe.device)
        if real_ref_count > 0:
            ref_msk[:, :real_ref_count] = 1.0

        full_msk = torch.cat([ref_msk, pose_msk], dim=1)
        full_latent = torch.cat([ref_latents, pose_latents], dim=1)
        y = torch.cat([full_msk, full_latent], dim=0).unsqueeze(0)
        return {"y": y}
...
class WanVideoUnit_OmniShow_Audio(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("is_omnishow_enable_audio", "input_audio", "audio_sample_rate", "num_frames"),
            output_params=("omnishow_audio_embeds",),
            onload_model_names=("audio_encoder",),
        )

    def process(self, pipe: WanVideoPipeline, is_omnishow_enable_audio, input_audio, audio_sample_rate, num_frames):
        if not is_omnishow_enable_audio:
            return {}
        if num_frames is None:
            return {}

        # Prepare OmniShow audio embeddings on the latent-frame timeline.
        num_latent_frames = (int(num_frames) - 1) // 4 + 1

        packed_dim = 13 * 768
        window_size = 5
        if pipe.audio_encoder is not None and hasattr(pipe.audio_encoder, "packed_dim"):
            packed_dim = int(getattr(pipe.audio_encoder, "packed_dim"))
        if pipe.audio_encoder is not None and hasattr(pipe.audio_encoder, "window_size"):
            window_size = int(getattr(pipe.audio_encoder, "window_size"))

        if input_audio is None or pipe.audio_encoder is None or not hasattr(pipe.audio_encoder, "encode"):
            audio_embeds = torch.zeros(
                (1, num_latent_frames, window_size, packed_dim), dtype=pipe.torch_dtype, device=pipe.device
            )
            return {"omnishow_audio_embeds": audio_embeds}
        ...
        if torch.is_tensor(input_audio):
            ...
            waveform = torch.as_tensor(input_audio)
        if waveform.dim() == 1:
            waveform = waveform.unsqueeze(0)
        waveform = waveform.to(device=pipe.device, dtype=torch.float32)

        audio_embeds = pipe.audio_encoder.encode(waveform, num_latent_frames)
        audio_embeds = audio_embeds.to(device=pipe.device, dtype=pipe.torch_dtype)
        return {"omnishow_audio_embeds": audio_embeds}
...
def model_fn_wan_video(
    ...
    vace_scale = 1.0,
    audio_embeds: Optional[torch.Tensor] = None,
    omnishow_audio_embeds: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            omnishow_audio_embeds=omnishow_audio_embeds,
            tea_cache=tea_cache,
    ...
    # Prepare projected OmniShow audio tokens for local attention.
    audio_tokens = None
    video_frame_lens = None
    audio_frame_lens = None
    if omnishow_audio_embeds is not None and hasattr(dit, "omnishow_audio_projector") and dit.omnishow_audio_projector is not None:
        if omnishow_audio_embeds.dim() != 4:
            raise ValueError(
                f"omnishow_audio_embeds must be (B,T_lat,w,packed_dim), got {tuple(omnishow_audio_embeds.shape)}"
            )
        B = x.shape[0]
        if omnishow_audio_embeds.shape[0] != B:
            raise ValueError(
                f"omnishow_audio_embeds batch mismatch: expected {B}, got {omnishow_audio_embeds.shape[0]}"
            )
        _, t_lat, w_win, packed_dim = omnishow_audio_embeds.shape
        if not hasattr(dit, "dim"):
            raise ValueError("dit must expose `dim` for OmniShow-GLCA")
        total_frames = int(f)
        n_ref = total_frames - int(t_lat)
        if n_ref < 0:
            raise ValueError(
                f"invalid OmniShow alignment: total_frames={total_frames} < t_lat={int(t_lat)}"
            )
        hw = int(h) * int(w)
        video_frame_lens = torch.full((B, total_frames), hw, device=x.device, dtype=torch.int32)
        if n_ref > 0:
            audio_frame_lens = torch.cat(
                [
                    torch.zeros((B, n_ref), device=x.device, dtype=torch.int32),
                    torch.full((B, int(t_lat)), int(w_win), device=x.device, dtype=torch.int32),
                ],
                dim=1,
            )
        else:
            audio_frame_lens = torch.full((B, int(t_lat)), int(w_win), device=x.device, dtype=torch.int32)

        audio_flat = omnishow_audio_embeds.to(device=x.device, dtype=x.dtype).reshape(B, int(t_lat) * int(w_win), int(packed_dim))
        audio_flat = dit.omnishow_audio_projector(audio_flat)
        audio_tokens = audio_flat.reshape(B, int(t_lat), int(w_win), int(dit.dim))
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                x = gradient_checkpoint_forward(
                    ...
                    video_frame_lens=video_frame_lens,
                    audio_frame_lens=audio_frame_lens,
                )
